...
void OLED_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8|GPIO_Pin_9;	  //PB8->IIC_SCL  PB9->IIC_SDA
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD; 		    //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;       //速度50MHz
 	GPIO_Init(GPIOB, &GPIO_InitStructure);	                //初始化PA0,PA1
 	GPIO_SetBits(GPIOB,GPIO_Pin_8|GPIO_Pin_9);              //PA0,PA1置高
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
		    ...
		    OLED_ShowChar(x+(size2/2)*t,y,' ',size2,1);   //如果没到数字就显示空格
			continue;
       ...
       //判断是否为最后一位的前一位（显示一位小数）
		t++;
	 	...
	 	OLED_ShowChar(x+(size2/2)*t,y,temp+'0',size2,1); //一位一位显示下去
	 }
}
